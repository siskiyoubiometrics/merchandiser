# Run with Rscript bench/benchmark-optimize-distinct.R 1000.
# Timing includes public input preparation, bucking, scaling, and result assembly.
args <- commandArgs(trailingOnly = TRUE)
stems <- if (length(args)) as.integer(args[1]) else 1000L
...
cat("Diameters are distinct across the run. Heights retain the nine-step cycle.\n")
  ...
  for (strategy in c("cascade", "optimize")) {
    ...
    elapsed <- system.time({
      for (first in seq.int(1L, stems, by = 1000L)) {
        ...
        result <- merchandise(ids, 18 + 4 * (ids - 1) / max(1, stems - 1), 76 + ids %% 9L, 202, p,
...
write.csv(results, Sys.getenv("MERCH_BENCH_CSV", "/tmp/benchmark-optimize-distinct.csv"), row.names = FALSE)
