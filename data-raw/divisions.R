# Regenerate the NSVB ecological division polygons used by nsvb_division_xy().
#
# The NSVB codes retain the 2007 ECOMAP hierarchy. Mountain divisions use an
# M prefix in the source, encoded by treevolume as 1,000 plus the numeric code.
# ECOMAP 2025 restructures the codes and does not contain those mountain
# divisions, so it is not compatible with the NSVB coefficient tables.
#
# Run from the package root. sf is used only while regenerating package data.
...
source_root <- Sys.getenv(
  "TREEVOLUME_EXTERNAL",
  unset = file.path("..", "merchandiser", "tools", "external")
)
source_zip <- file.path(source_root, source_name)
if (!file.exists(source_zip)) {
  stop("missing ECOMAP source archive: ", source_zip)
}
actual_sha256 <- strsplit(
  system2("sha256sum", source_zip, stdout = TRUE), " ", fixed = TRUE
)[[1L]][1L]
...
shape_path <- list.files(
  extract_dir, pattern = "[.]shp$", full.names = TRUE, ignore.case = TRUE
)
...
if (!all(provinces$MAP_LEVEL == "PROV") ||
      !all(provinces$PROJECT == "ECOMAP07")) {
...
provinces$division <- numeric_code %/% 10L * 10L +
  ifelse(mountain, 1000L, 0L)
...
simplified <- sf::st_simplify(
  divisions, dTolerance = simplification_tolerance_m,
  preserveTopology = TRUE
)
simplified$geometry <- sf::st_cast(
  sf::st_geometry(simplified), "MULTIPOLYGON", warn = FALSE
)
area_difference_pct <- 100 * abs(
  as.numeric(sf::st_area(simplified)) - original_area
) / original_area
...
ring_key <- paste(
  coordinates[, "L3"], coordinates[, "L2"], coordinates[, "L1"], sep = ":"
)
nsvb_division_polygons <- data.frame(
  division = as.integer(simplified$division[coordinates[, "L3"]]),
  ring_id = as.integer(match(ring_key, unique(ring_key))),
  hole = coordinates[, "L1"] > 1,
  x = as.double(coordinates[, "X"]),
  y = as.double(coordinates[, "Y"]),
...
attr(nsvb_division_polygons, "simplification_tolerance_m") <-
  simplification_tolerance_m
attr(nsvb_division_polygons, "maximum_area_difference_pct") <-
  max(area_difference_pct)
...
save(
  ...
  file = file.path("data", "nsvb_division_polygons.rda"),
  compress = "xz", version = 3L
)
dir.create(file.path("tests", "testthat", "fixtures"), recursive = TRUE,
           showWarnings = FALSE)
utils::write.csv(
  nsvb_division_points,
  file.path("tests", "testthat", "fixtures", "nsvb_division_points.csv"),
  row.names = FALSE
)
...
save(
  species_reference, nsvb_division_polygons,
  file = file.path("R", "sysdata.rda"), compress = "xz", version = 3L
)
...
provenance <- data.frame(
  ...
  source_code_field = "MAP_UNIT_S",
  source_feature_count = nrow(provinces),
  ...
  maximum_area_difference_pct = max(area_difference_pct),
  polygon_table_rows = nrow(nsvb_division_polygons),
  compressed_data_bytes = file.info(
    file.path("data", "nsvb_division_polygons.rda")
  )$size,
  stringsAsFactors = FALSE
)
utils::write.csv(
  provenance,
  file.path("inst", "extdata", "nsvb_division_provenance.csv"),
  row.names = FALSE
)

message(
  "Wrote ", length(division_codes), " divisions with ",
  nrow(nsvb_division_polygons), " coordinate rows. The ",
  simplification_tolerance_m, " m tolerance retained ", simplified_vertices,
  " of ", original_vertices, " vertices. Maximum area difference: ",
  format(max(area_difference_pct), digits = 12), " percent. Compressed data: ",
