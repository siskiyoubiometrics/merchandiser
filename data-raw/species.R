# Regenerate the NSVB species reference and exported species_reference data.
#
# The pinned NVEL species table supplies physical and NSVB classification
# fields. FIA REF_SPECIES supplies names and symbols when it is reachable.
# Package builds never make a network request. Set FIA_REF_SPECIES to a local
# downloaded CSV to perform the join reproducibly.
...
utils::write.csv(
  species_literals,
  file.path("inst", "extdata", "nsvb_species_literals.csv"),
  row.names = FALSE, na = ""
)

matrix_values <- matrix(species_literals$value, ncol = 12L, byrow = TRUE)
species_reference <- data.frame(
  spcd = as.integer(matrix_values[, 1L]),
  hardwood = as.integer(matrix_values[, 2L]),
  jenkins_group = as.integer(matrix_values[, 3L]),
  wood_dry_weight = matrix_values[, 4L],
  bark_dry_weight = matrix_values[, 5L],
  wood_moisture = matrix_values[, 6L],
  bark_moisture = matrix_values[, 7L],
  bark_to_wood_volume = matrix_values[, 8L],
  green_weight_factor = matrix_values[, 9L],
  dry_weight_factor = matrix_values[, 10L],
  sapling_adjustment = matrix_values[, 11L],
  carbon_ratio = matrix_values[, 12L],
  source_file = "wdbkwtdata.inc",
  source_chain = paste(
    "FIADB REF_SPECIES to NBEL BM_REF_SPECIES to NVEL wdbkwtdata.inc"
  ),
  source_last_modified = "2023-10-16",
  upstream_commit = nvel_commit,
  stringsAsFactors = FALSE
)
utils::write.csv(
  species_reference,
  file.path("inst", "extdata", "nsvb_species_reference.csv"),
  row.names = FALSE, na = ""
)
...
if (fia_available) {
  ...
  # TODO: Run data-raw/ref_species.R when the DataMart accepts the download,
  # set FIA_REF_SPECIES to that local CSV, and rerun this script.
...
species_reference <- data.frame(
  spcd = species_reference$spcd,
  symbol = symbol,
  common = common,
  scientific = scientific,
  genus = genus,
  softwood_hardwood = ifelse(
    species_reference$hardwood == 0L, "softwood", "hardwood"
  ),
  bark_ratio = sqrt(1 / (1 + species_reference$bark_to_wood_volume / 100)),
  wood_density = species_reference$wood_dry_weight,
  sources = paste(
    paste0(
      "FIADB REF_SPECIES to NBEL BM_REF_SPECIES to NVEL wdbkwtdata.inc, ",
...
# Keep the same generated object in the namespace so exported functions can
# use it without attaching the package data environment first.
if (file.exists(file.path("data", "nsvb_division_polygons.rda"))) {
  ...
  save(
    species_reference, file = file.path("R", "sysdata.rda"),
...
cpp_rows <- apply(matrix_values, 1L, function(row) {
  paste0("    Species{", paste(
    vapply(row, cpp_number, character(1)), collapse = ", "
  ), "},")
})
header <- c(
  "#ifndef TREEVOLUME_NSVB_SPECIES_HPP",
  "#define TREEVOLUME_NSVB_SPECIES_HPP",
  "",
  "// Generated by data-raw/species.R from NVEL commit",
...
writeLines(header, file.path(
  "inst", "include", "treevolume", "nsvb_species.hpp"
))
