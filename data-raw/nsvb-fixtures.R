# Create the small, committed NSVB fixture sample used in routine tests.
# The complete protected oracle remains outside the package and is exercised
...
nsvb_columns <- c(
  "ROW_ID", "FIXTURE_SET", "CALL_KIND", "REGN", "FORST", "DIST",
  "FIASPCD", "PROD", "CTYPE", "LIVE", "VOLEQ", "DBHOB", "HTTOT",
  "MTOPP", "MTOPS",
  "STUMP", "CULL", "DECAYCD", "MRULEMOD", "NEWMAXLEN", "NEWMINLEN",
  "NEWTRIM", "ERRFLAG", "HTUP", "STEMDIB", "CALCDIA_REQUESTED",
  "HT2TOPD_REQUESTED", "CALCDIA_ERRFLAG", "HT2TOPD_ERRFLAG", "DIB",
  "STEMHT", paste0("VOL", 1:15), "NOLOGP", "NOLOGS",
  paste0("DRYBIO", 1:15), paste0("GRNBIO", 1:15), paste0("BOLHT", 1:21)
)
...
select_main <- function(path) {
  ...
  values <- values[
    DBHOB == 12 & HTTOT == 80 &
      (
        grepl("^NVBC", CALL_KIND) |
          grepl("^VOLUMELIBRARY", CALL_KIND) |
...
anomaly_output <- file.path(
  destination, "nsvb_210_230.double.csv.gz"
)
...
stopifnot(sum(file.info(outputs)$size) < 3e5)
