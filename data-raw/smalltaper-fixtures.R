# Create the committed regional small-taper oracle sample. The complete fixtures remain
# outside the package and are tested through MERCHANDISER_FIXTURES.

fixture_root <- Sys.getenv("MERCHANDISER_FIXTURES", unset = file.path(
  "..", "merchandiser", "tools",
  "oracle", "fixtures"
))
families <- c("r1_taper", "r2_taper", "r5_taper", "r12_taper", "blm_taper", "behre_taper")

select_rows <- function(path) {
  values <- data.table::fread(path)
  values <- values[DBHOB == 12 & HTTOT == 80 & (CALL_KIND %in% c(
    "VOLUMELIBRARY",
    "VOLUMELIBRARY_MRULE"
  ) |
    (CALL_KIND == "PROFILE_PROBE" & CALCDIA_REQUESTED == "Y" & HTUP %in% c(1, 4.5, 40)) |
    (CALL_KIND == "PROFILE_PROBE" & HT2TOPD_REQUESTED == "Y" & STEMDIB %in% c(2, 6, 10)))]
