# Regenerate the Clark coefficient inventory, compile-time tables, model metadata, and
# pattern metadata.  Run from the package root. NVEL_SOURCE and MERCHANDISER_FIXTURES may
# override the read-only upstream checkouts. The installed package uses only the CSV
...
identifier_root <- Sys.getenv("TREEVOLUME_IDENTIFIERS", unset = file.path(
  "..", "merchandiser",
  "tools", "identifiers"
))

source_files <- c(
  "r8clkdib.f", "r8dib.f", "r8dib.inc", "r8clkcoef.inc", "r8cfo.inc", "r8clist.inc",
  "r8vlist.f", "r8vlist.inc", "r8init.f", "r8prep.f", "r8vol.f", "r8vol1.f",
  "r8vol2.f", "r9clarkdib.f",
  "r9clark.f", "r9coeff.inc", "r9init.f", "r9vol.f", "r9logs.f", "clkcoef_mod.f",
  "calcdia.f",
  "ht2topd.f", "profile.f", "volinit.f", "voleqdef.f"
)
...
routine_at <- function(lines, line_number) {
  ...
  declarations <- grep("^[ ]{0,6}(subroutine|([a-z0-9*]+[ ]+)?function)[ ]+", preceding,
    ignore.case = TRUE,
    value = TRUE
  )
  ...
  sub("^.*(?:subroutine|function)[ ]+([a-z0-9_]+).*$", "\\1", declaration,
    ignore.case = TRUE
  )
}
...
parse_literal <- function(literal) {
  if (grepl("^(['\"]).*\\1$", literal)) {
    return(list(type = "character", numeric = NA_real_, character = substring(
      literal, 2L,
      nchar(literal) - 1L
    )))
  }
  if (tolower(literal) %in% c(".true.", ".false.")) {
    return(list(type = "logical", numeric = NA_real_, character = tolower(literal)))
  }
...
parse_file <- function(file) {
  ...
  while (line_number <= length(lines)) {
    ...
    matched <- regexec("^[ ]*data[ ]*([^/]+)/(.+)/[ ]*$", statement, ignore.case = TRUE)
    ...
    rows[[length(rows) + 1L]] <- data.frame(
      source_file = file, source_line_start = start,
      source_line_end = line_number, routine = routine_at(lines, start), target = target,
      ordinal = seq_along(literals), literal = literals, value_type = vapply(
        parsed, `[[`,
        character(1), "type"
...
utils::write.csv(clark_coefficients, file.path("inst", "extdata", "clark_coefficients.csv"),
  row.names = FALSE, na = ""
)
coefficient_round_trip <- utils::read.csv(
  file.path(
    "inst", "extdata",
    "clark_coefficients.csv"
  ),
  stringsAsFactors = FALSE, na.strings = ""
)
if (!isTRUE(all.equal(clark_coefficients, coefficient_round_trip,
...
table_values <- function(source_file, target_pattern) {
  selected <- clark_coefficients$source_file == source_file & grepl(target_pattern,
    clark_coefficients$target,
    ignore.case = TRUE
  )
...
compiled_tables <- list(r8_cf = table_values("r8dib.inc", "R8CF\\("), r8_cfo = table_values(
  "r8cfo.inc",
  "R8CFO\\("
), r8_dibmen = table_values("r8clkcoef.inc", "DIBMEN\\("), r8_total = table_values(
  "r8clkcoef.inc",
  "^[(][(]TOTAL\\("
), r8_ototal = table_values("r8clkcoef.inc", "^[(][(]OTOTAL\\("), r8_four = table_values(
  "r8clkcoef.inc",
  "FOUR\\("
), r8_seven = table_values("r8clkcoef.inc", "SEVEN\\("), r8_nine = table_values(
  "r8clkcoef.inc",
  "NINE\\("
), r9_a = table_values("r9coeff.inc", "coefA\\("), r9_0 = table_values(
  "r9coeff.inc",
  "coef0\\("
), r9_4 = table_values("r9coeff.inc", "coef4\\("), r9_79 = table_values(
  "r9coeff.inc",
  "coef79\\("
))

expected_lengths <- c(
  r8_cf = 182L * 18L, r8_cfo = 182L * 9L, r8_dibmen = 49L * 3L,
  r8_total = 49L *
    7L, r8_ototal = 49L * 7L, r8_four = 49L * 6L, r8_seven = 15L * 6L, r8_nine = 34L *
    6L, r9_a = 47L *
    4L, r9_0 = 47L * 9L, r9_4 = 47L * 8L, r9_79 = 47L * 8L
)
...
header <- c(
  "#ifndef TREEVOLUME_CLARK_COEFFICIENTS_HPP", "#define TREEVOLUME_CLARK_COEFFICIENTS_HPP",
  "", "// Generated by data-raw/clark.R from NVEL commit", paste0(
    "// ",
    nvel_commit, ". Do not edit by hand."
  ),
  "namespace treevolume {", "namespace clark_coefficients {", ""
)
for (name in names(compiled_tables)) {
  header <- c(header, paste0("inline constexpr double ", name, "[] = {"), paste0(
    "    ", paste(vapply(
      compiled_tables[[name]],
      cpp_number, character(1)
    ), collapse = ", "), "};"
  ), "")
}
header <- c(
  header, "}  // namespace clark_coefficients", "}  // namespace treevolume", "", "#endif",
  ""
)
writeLines(header, file.path("inst", "include", "merchandiser", "clark_coefficients.hpp"))
...
clark_identifiers <- utils::read.csv(identifier_path,
  stringsAsFactors = FALSE,
...
clark_models <- data.frame(
  id = clark_identifiers$identifier, family = clark_identifiers$family,
  species = as.integer(clark_identifiers$table_species), region = as.integer(
    clark_identifiers$REGN
  ),
  forest = sprintf("%02d", as.integer(clark_identifiers$FORST)), district = sprintf(
    "%02d",
    as.integer(clark_identifiers$DIST)
  ), variant = clark_identifiers$VAR, product = sprintf(
    "%02d",
    as.integer(clark_identifiers$PROD)
  ), top_code = ifelse(clark_identifiers$REGN == 8L,
...
utils::write.csv(clark_models, file.path("inst", "extdata", "clark_models.csv"),
  row.names = FALSE,
  na = ""
)
...
clark_patterns <- patterns[
  patterns$family %in% c("clark_r8", "clark_r9") & grepl(
    "[?]", patterns$voleq
  ),
...
utils::write.csv(clark_patterns, file.path("inst", "extdata", "clark_patterns.csv"),
  row.names = FALSE,
  na = ""
)

message(
  "Wrote ", nrow(clark_coefficients), " Clark DATA literal rows, ", nrow(clark_models),
  " models, and ", nrow(clark_patterns), " patterns."
)
