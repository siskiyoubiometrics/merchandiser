# Shared source extractor for the regional small-taper families.  Run a family script from
# the package root. NVEL_SOURCE may override the read-only checkout, and
# MERCHANDISER_FIXTURES may override the oracle root.

.smalltaper_commit <- "38548071d5aa652bb90c7f111f86b427f798a1c9"
.smalltaper_source <- Sys.getenv("NVEL_SOURCE", unset = file.path(
  "..", "..", "tools", "oracle",
  "nvel"
))

.smalltaper_files <- list(
  r1_taper = "r1tap.f", r2_taper = "r2tap.f",
  r5_taper = "r5tap.f", r12_taper = c(
    "r12tap.f",
    "stump.f"
  ), blm_taper = c("blmtap.f", "blmvol.f", "formclas.f"), behre_taper = c(
    "blmtap.f",
    "formclas.f", "r6vol3.f"
  )
)
...
.smalltaper_routine <- function(lines, line_number) {
  declarations <- grep("^[ ]{0,6}(subroutine|([a-z0-9*]+[ ]+)?function)[ ]+",
    lines[seq_len(line_number)],
    ignore.case = TRUE, value = TRUE
  )
  if (!length(declarations))
    return(NA_character_)
  sub("^.*(?:subroutine|function)[ ]+([a-z0-9_]+).*$", "\\1", tail(
    declarations,
    1L
  ), ignore.case = TRUE)
}
...
.smalltaper_expand <- function(values) {
  unlist(lapply(values[nzchar(values)], function(value) {
    ...
    if (length(parts))
      rep(trimws(parts[[3L]]), as.integer(parts[[2L]])) else value
  }), use.names = FALSE)
}
...
.smalltaper_parse_file <- function(source_file) {
  ...
  while (line_number <= length(lines)) {
    ...
    if (.smalltaper_comment(line) || !grepl("^[ ]{0,6}data(?:[ ]|[(])", line,
          ignore.case = TRUE,
          perl = TRUE
        )) {
    ...
    parts <- regmatches(statement, regexec("^[ ]*data[ ]*([^/]+)/(.+)/[ ]*$", statement,
      ignore.case = TRUE
    ))[[1L]]
    if (!length(parts))
      stop(source_file, ":", start, ": could not parse DATA")
    ...
    rows[[length(rows) + 1L]] <- data.frame(
      ...
      source_line_end = line_number, routine = .smalltaper_routine(lines, start),
...
.write_smalltaper_metadata <- function() {
  ...
  rows <- lapply(families, function(family) {
    ...
    values <- unique(utils::read.csv(path, colClasses = classes, stringsAsFactors = FALSE))
    names(values) <- c("species", "id")[match(names(values), c("FIASPCD", "VOLEQ"))]
    values$family <- family
    values$source_file <- paste0("MERCHANDISER_FIXTURES/full/", family, ".double.csv.gz")
  ...
  utils::write.csv(metadata, file.path("inst", "extdata", "smalltaper_models.csv"),
    row.names = FALSE,
    na = ""
  )
}
...
extract_smalltaper_family <- function(family) {
  if (!family %in% names(.smalltaper_files))
    stop("unknown family: ", family)
  missing <- .smalltaper_files[[family]][!file.exists(file.path(
    .smalltaper_source,
    .smalltaper_files[[family]]
  ))]
  if (length(missing))
    stop("missing NVEL source files: ", paste(missing, collapse = ", "))
  actual_commit <- system2("git", c(
    "-C", normalizePath(.smalltaper_source),
    "rev-parse", "HEAD"
  ),
  stdout = TRUE
  )
  ...
  coefficients <- do.call(rbind, lapply(
    .smalltaper_files[[family]],
    .smalltaper_parse_file
  ))
  rownames(coefficients) <- NULL
  dir.create(file.path("inst", "extdata"), recursive = TRUE, showWarnings = FALSE)
  utils::write.csv(coefficients, file.path("inst", "extdata", paste0(
    family,
    "_coefficients.csv"
  )),
  row.names = FALSE, na = ""
