# Regenerate the Flewelling coefficient inventory and the compile-time table.  Run from the
# package root. Set NVEL_SOURCE to override the read-only NVEL checkout. The package does
# not require this script or an R data object at run time. CSV is used so every source
# literal and its provenance remain directly inspectable, while the generated header
# supplies immutable values to the compiled kernel.
...
source_files <- c(
  "f_west.f", "f_ingy.f", "f_alaska.f", "f_other.f", "sf_2pt.f", "sf_2pth.f",
  "sf_3pt.f", "sf_3z.f", "sf_corr.f", "sf_dfz.f", "sf_ds.f", "sf_hs.f", "sf_shp.f",
  "sf_taper.f",
...
utils::write.csv(flewelling_coefficients, file.path(
  "inst", "extdata",
  "flewelling_coefficients.csv"
),
row.names = FALSE, na = ""
)
...
statement_key <- interaction(flewelling_coefficients$source_file,
...
header <- c(
  ...
  "", "// Generated by data-raw/flewelling.R from NVEL commit", paste0(
    "// ", nvel_commit,
    ". Do not edit by hand."
  ), "namespace treevolume {", "namespace flewelling_coefficients {",
  ""
)
for (statement in statements) {
  ...
  header <- c(
    header, paste0("// ", statement$routine[[1L]], ": ", statement$target[[1L]]),
    paste0("inline constexpr double ", name, "[] = {"), paste0("    ", paste(vapply(
      statement$value_numeric,
      cpp_number, character(1)
    ), collapse = ", "), "};"), ""
  )
}
header <- c(
  header, "}  // namespace flewelling_coefficients", "}  // namespace treevolume",
  "", "#endif", ""
)
writeLines(header, file.path(
  "inst", "include", "merchandiser",
  "flewelling_coefficients.hpp"
))
...
model_paths <- file.path(fixture_full, c(
  "flewelling_2pt.double.csv.gz",
  "flewelling_3pt.double.csv.gz"
))
...
flewelling_models <- flewelling_models[order(
...
utils::write.csv(flewelling_models, file.path("inst", "extdata", "flewelling_models.csv"),
  row.names = FALSE,
  na = ""
)
