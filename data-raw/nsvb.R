# Regenerate NSVB coefficient, division, regional-weight, and model tables.
#
# Run from the package root. Set NVEL_SOURCE and MERCHANDISER_FIXTURES to
# override the read-only reference checkouts. Every stored literal carries its
# source file, source line range, and pinned upstream commit. The generated C++
# header is the immutable run-time representation used by the compiled kernel.
...
source_files <- c(
  paste0("tables", 1:11, ".inc"), "dist_ecoprov.inc", "regndftdata.inc"
)
reference_files <- c(source_files, "scrib.f")
missing_files <- reference_files[
  !file.exists(file.path(nvel_source, reference_files))
]
...
parse_numeric_data <- function(source_file, targets) {
  ...
  while (line_number <= length(lines)) {
    ...
    if (is_comment(line) || !grepl("^[ ]{0,7}DATA[ ]*\\(\\(", line,
                                   ignore.case = TRUE)) {
...
specification <- list(
  tables1.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables2.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables3.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables4.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables5.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables6.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables7.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables8.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables9.inc = c(SPcoef = 13L, JKcoef = 5L),
  tables10.inc = c(SPCF = 2L),
  tables11.inc = c(DIVCRh = 2L, DIVCRs = 2L),
...
literal_rows <- lapply(names(specification), function(source_file) {
  ...
  column_count <- unname(specification[[source_file]][match(
    target_lower, tolower(names(specification[[source_file]]))
  )])
...
expected <- c(
  "tables1.inc:spcoef" = 406L * 13L,
  "tables1.inc:jkcoef" = 9L * 5L,
  "tables2.inc:spcoef" = 339L * 13L,
  "tables2.inc:jkcoef" = 9L * 5L,
  "tables3.inc:spcoef" = 361L * 13L,
  "tables3.inc:jkcoef" = 9L * 5L,
  "tables4.inc:spcoef" = 361L * 13L,
  "tables4.inc:jkcoef" = 9L * 5L,
  "tables5.inc:spcoef" = 405L * 13L,
  "tables5.inc:jkcoef" = 9L * 5L,
  "tables6.inc:spcoef" = 206L * 13L,
  "tables6.inc:jkcoef" = 9L * 5L,
  "tables7.inc:spcoef" = 175L * 13L,
  "tables7.inc:jkcoef" = 9L * 5L,
  "tables8.inc:spcoef" = 173L * 13L,
  "tables8.inc:jkcoef" = 9L * 5L,
  "tables9.inc:spcoef" = 201L * 13L,
  "tables9.inc:jkcoef" = 9L * 5L,
  "tables10.inc:spcf" = 460L * 2L,
  "tables11.inc:divcrh" = 43L * 2L,
  "tables11.inc:divcrs" = 43L * 2L,
  "dist_ecoprov.inc:distprov" = 505L * 2L,
  "dist_ecoprov.inc:forstprov" = 108L * 2L,
  "dist_ecoprov.inc:regnprov" = 9L * 2L,
  "regndftdata.inc:spregndftwf" = 149L * 7L
)
keys <- paste(nsvb_coefficients$source_file,
              tolower(nsvb_coefficients$target), sep = ":")
...
utils::write.csv(
  nsvb_coefficients,
  file.path("inst", "extdata", "nsvb_coefficients.csv"),
  row.names = FALSE, na = ""
)

# NVBC delegates its two board-foot calculations to SCRIB and INTL14. Preserve
# SCRIB's factor and correction tables separately so their provenance is as
# explicit as the NSVB supplement tables without changing the supplement-row
# invariant above.
parse_scrib_data <- function(target, expected_count) {
  source_file <- "scrib.f"
  lines <- readLines(file.path(nvel_source, source_file), warn = FALSE)
  starts <- grep(
    paste0("^[ ]{0,7}DATA[ ]*\\(", target, "\\(I\\)"),
    lines, ignore.case = TRUE
  )
  rows <- lapply(starts, function(start) {
    finish <- start
    statement_lines <- lines[[finish]]
    slash_count <- lengths(regmatches(
      statement_lines, gregexpr("/", statement_lines, fixed = TRUE)
    ))
    while (slash_count < 2L && finish < length(lines)) {
      finish <- finish + 1L
      statement_lines <- c(statement_lines, lines[[finish]])
      slash_count <- slash_count + lengths(regmatches(
        lines[[finish]], gregexpr("/", lines[[finish]], fixed = TRUE)
      ))
    }
    statement <- paste(substring(statement_lines, 7L), collapse = " ")
...
scribner_coefficients <- rbind(
  parse_scrib_data("FACTOR", 132L),
  parse_scrib_data("EXCEPT", 149L)
)
rownames(scribner_coefficients) <- NULL
utils::write.csv(
  scribner_coefficients,
  file.path("inst", "extdata", "nsvb_scribner_coefficients.csv"),
  row.names = FALSE, na = ""
)

scribner_rows <- function(target) {
  values <- scribner_coefficients$value[
    tolower(scribner_coefficients$target) == tolower(target)
  ]
  paste0("    ", vapply(values, function(value) sprintf("%.17g", value),
                        character(1)), ",")
}
scribner_header <- c(
  "#ifndef TREEVOLUME_NSVB_SCRIBNER_HPP",
  "#define TREEVOLUME_NSVB_SCRIBNER_HPP",
  "",
  "// Generated by data-raw/nsvb.R from NVEL scrib.f at commit",
  paste0("// ", nvel_commit, ". Do not edit by hand."),
  "namespace treevolume {",
  "namespace nsvb_data {",
  "inline constexpr double scribner_factor[] = {",
  scribner_rows("FACTOR"),
  "};",
  "inline constexpr double scribner_exception[] = {",
...
writeLines(scribner_header, file.path(
  "inst", "include", "merchandiser", "nsvb_scribner.hpp"
))

target_matrix <- function(source_file, target, columns) {
  selected <- nsvb_coefficients$source_file == source_file &
    tolower(nsvb_coefficients$target) == tolower(target)
...
cpp_rows <- function(matrix_value, struct_name) {
  apply(matrix_value, 1L, function(row) {
    paste0("    ", struct_name, "{", paste(
      vapply(row, cpp_number, character(1)), collapse = ", "
    ), "},")
  })
}

header <- c(
  "#ifndef TREEVOLUME_NSVB_COEFFICIENTS_HPP",
  "#define TREEVOLUME_NSVB_COEFFICIENTS_HPP",
  "",
  "#include <cstddef>",
  "",
  "// Generated by data-raw/nsvb.R from NVEL commit",
  paste0("// ", nvel_commit, ". Do not edit by hand."),
  "namespace treevolume {",
  "namespace nsvb_data {",
  "",
  ...
  "struct Pair { double key; double value; };",
  "struct RegionWeight {",
...
for (table_number in 1:9) {
  ...
  header <- c(
    header,
    paste0("inline constexpr EquationCoefficient ", table_name, "[] = {"),
    cpp_rows(spcoef, "EquationCoefficient"),
    "};",
    paste0("inline constexpr GroupCoefficient ", table_name, "_group[] = {"),
...
pair_tables <- list(
  ...
  crown_hardwood = target_matrix("tables11.inc", "DIVCRh", 2L),
  crown_softwood = target_matrix("tables11.inc", "DIVCRs", 2L),
  district_province = target_matrix("dist_ecoprov.inc", "DistProv", 2L),
  forest_province = target_matrix("dist_ecoprov.inc", "ForstProv", 2L),
  region_province = target_matrix("dist_ecoprov.inc", "RegnProv", 2L)
)
for (name in names(pair_tables)) {
  header <- c(
    header, paste0("inline constexpr Pair ", name, "[] = {"),
    cpp_rows(pair_tables[[name]], "Pair"), "};", ""
  )
}
region_weight <- target_matrix("regndftdata.inc", "SPREGNDFTWF", 7L)
header <- c(
  header, "inline constexpr RegionWeight region_weight[] = {",
  cpp_rows(region_weight, "RegionWeight"), "};", "",
...
writeLines(header, file.path(
  "inst", "include", "merchandiser", "nsvb_coefficients.hpp"
))

division_rows <- do.call(rbind, Map(function(name, values) {
  data.frame(
    level = name,
    key = as.integer(values[, 1L]),
    division = as.integer(values[, 2L]),
    source_file = "dist_ecoprov.inc",
    upstream_commit = nvel_commit,
    stringsAsFactors = FALSE
  )
}, c("district", "forest", "region"), pair_tables[
  c("district_province", "forest_province", "region_province")
]))
utils::write.csv(
  division_rows, file.path("inst", "extdata", "nsvb_divisions.csv"),
  row.names = FALSE, na = ""
)

# SPEC_INTERFACE.md defines the public division helper in terms of State and
# county FIPS codes. NVEL itself contains only the National Forest district
# crosswalk above, so the public lookup is extracted from the Forest Service's
# county ecological-subregion table. The PDF is checksum-pinned and is never
# fetched during a package build.
...
if (!nzchar(ecocode_pdf) || !file.exists(ecocode_pdf)) {
  stop(
    "set ECOCODE_PDF to the official gtr_srs036.pdf downloaded from ",
    ecocode_url
  )
}
...
matches <- regmatches(
  ecocode_lines, regexec(county_pattern, ecocode_lines, perl = TRUE)
)
...
nsvb_county_divisions <- data.frame(
  state = as.integer(substr(fips, 1L, 2L)),
  county = as.integer(substr(fips, 3L, 5L)),
  division = as.integer(county_values[, 7L]) +
    ifelse(county_values[, 6L] == "2", 1000L, 0L),
  mountain = as.integer(county_values[, 6L]),
  province = as.integer(county_values[, 7L]),
  section = as.integer(county_values[, 8L]),
  subregion_code = as.integer(county_values[, 5L]),
  predominant_area_percent = as.double(county_values[, 9L]),
  source_page = ecocode_pages[matched],
  stringsAsFactors = FALSE
)
if (anyDuplicated(paste(nsvb_county_divisions$state,
                        nsvb_county_divisions$county))) {
  stop("county ecological-subregion table contains duplicate FIPS keys")
}
utils::write.csv(
  nsvb_county_divisions,
  file.path("inst", "extdata", "nsvb_county_divisions.csv"),
  row.names = FALSE, na = ""
)
utils::write.csv(
  data.frame(
    source_file = "gtr_srs036.pdf",
    source_url = ecocode_url,
    source_table = "Table 1, PDF pages 21 through 94",
    sha256 = ecocode_sha256,
    rows = nrow(nsvb_county_divisions),
    stringsAsFactors = FALSE
  ),
  file.path("inst", "extdata", "nsvb_county_divisions_provenance.csv"),
  row.names = FALSE
)

read_columns <- function(path, columns) {
  connection <- if (grepl("[.]gz$", path)) gzfile(path, open = "rt") else file(path)
...
anomaly_path <- file.path(
  fixture_root, "anomalies", "nsvb_210_230_representative.double.csv"
)
...
nsvb_models$source_file <- ifelse(
  substr(nsvb_models$id, 5L, 7L) %in% c("210", "230"),
...
utils::write.csv(
  nsvb_models, file.path("inst", "extdata", "nsvb_models.csv"),
  row.names = FALSE, na = ""
)

message(
  "Wrote ", nrow(nsvb_coefficients), " NSVB numeric literals and metadata for ",
  nrow(nsvb_models), " models."
)
