# Create the committed Region 10 and Region 4 fixture sample. The complete corpora remain
# external and are exercised when MERCHANDISER_FIXTURES is set.
...
select_rows <- function(values, family) {
  profile <- values$CALL_KIND == "PROFILE_PROBE" & values$DBHOB == 12 & values$HTTOT == 80 &
    (values$HTUP %in% c(0, 1, 4.5, 40) | (values$HT2TOPD_REQUESTED == "Y" &
                                            values$STEMDIB %in%
                                              c(2, 6, 10)))
  volume <-
    values$CALL_KIND %in% c("VOLUMELIBRARY", "VOLUMELIBRARY_MRULE") &
  ...
  anomaly <- if (identical(family, "r10_taper")) {
    values$VOLEQ == "A16BRUW042" & values$CALL_KIND == "VOLUMELIBRARY" & values$DBHOB ==
      10 & values$HTTOT == 80
  } else {
      ...
      (values$STEMHT == 0 |
         (values$VOLEQ == "405MATW202" &
            ((values$DBHOB == 4 & values$HTTOT == 100 & values$STEMDIB == 2) |
               (values$DBHOB == 6 & values$HTTOT == 160 & values$STEMDIB == 2))))
  }
...
for (precision in c("single", "double")) {
  for (family in c("r10_taper", "r4_driver")) {
    ...
    values <- utils::read.csv(input, stringsAsFactors = FALSE, check.names = FALSE)
