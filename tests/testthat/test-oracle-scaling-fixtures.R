test_that("full native NVEL log tables agree with verified oracle fixtures", {
  ...
  expect_identical(
    agreement$source_rows_scanned,
    agreement$rows_compared + agreement$excluded_not_volumelibrary +
      agreement$excluded_oracle_error + agreement$excluded_no_log_table
  )
  expect_identical(
    agreement$source_logs,
    agreement$included_logs + agreement$excluded_missing_log_fields
  )
  f32 <- agreement$family == "flewelling_3pt"
  expect_identical(
    agreement$included_logs[f32],
    agreement$scribner_source_logs[f32] +
      agreement$excluded_f32_unpaired_logs[f32] +
  ...
  expect_identical(
    agreement$scribner_within_tolerance, agreement$scribner_compared
  )
  expect_identical(
  ...
  expected <- data.frame(
    family = c(
      "flewelling_2pt", "flewelling_3pt", "clark_r8", "clark_r9"
    ),
    excluded_not_volumelibrary = c(165874, 48505, 2353547, 729339),
    excluded_oracle_error = c(868, 0, 207030, 7262),
    excluded_no_log_table = c(4215, 1616, 414021, 75849),
    excluded_missing_log_fields = c(0, 0, 0, 54),
    excluded_f32_unpaired_logs = c(0, 573, 0, 0),
    excluded_f32_incomplete_logs = c(0, 0, 0, 0),
    excluded_scribner_non_split_logs = c(0, 0, 0, 1240290),
    excluded_international_family = c(0, 0, 207083, 173488),
    excluded_international_incomplete = c(0, 0, 0, 0),
    invalid_build_rows = c(0, 0, 0, 0),
  ...
  for (reason in reason_columns) {
    expect_identical(
      agreement[[reason]], expected[[reason]][target],
      info = paste("exact exclusion count for", reason)
    )
  }
  ...
  rules <- merchandiser::nvel_rules(
    ...
    minimum_length = 2, minimum_top_length = 2,
    merchantable_length = case$BOLHT2 - case$STUMP,
    primary_top = case$MTOPP, secondary_top = case$MTOPS,
    stump = case$STUMP, trim = trim, bark_ratio = NA_real_,
    minimum_board_foot_dbh = 1, scribner = "table", prod = "01",
    ht_type = "F", live = "L", ctype = "C", forest = 0, district = 1
  )
  translated <- products_from_nvel_rules(rules)
  translated$products$price[] <- 1
  run <- function(products) {
    merchandise(
      as.double(case$DBHOB), as.double(case$HTTOT), case$VOLEQ, products,
      spcd = as.integer(case$FIASPCD), stump_ht = as.double(case$STUMP),
      utilization_height = as.double(case$BOLHT2), currency = "USD",
      status = TRUE
    )
  }
  scribner <- run(translated$products)
  expect_identical(scribner$trees$status, 0L)
  expect_identical(scribner$logs$nominal_length, as.double(case$LOGLEN1))
  expect_identical(scribner$logs$gross_scale, as.double(case$LOGVOL_1_1))
  international_products <- translated$products
  international_products$scale_rule[] <- "international_1_4_4ft"
  international <- run(international_products)
  expect_identical(international$trees$status, 0L)
  expect_identical(
    international$logs$nominal_length, as.double(case$LOGLEN1)
  )
  expect_identical(international$logs$gross_scale, as.double(case$VOL10))
