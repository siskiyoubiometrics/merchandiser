test_that("inversion refinement covers exact and failed midpoint branches", {
  model <- new_taper_model(id = "private.refine", form = "test", dib = linear_dib)
  exact <- merchandiser:::.refine_crossing_brackets(
    model, "dib", 10, 80, 5, list(), 1L, 39,
    41, linear_dib(10, 80, 39, list()) - 5
  )
  ...
  failed_model <- new_taper_model(
    id = "private.refine_failure", form = "test",
    dib = failed_dib
  )
  failed <- merchandiser:::.refine_crossing_brackets(
    failed_model, "dib", 10, 80, 5, list(),
    1L, 39, 41, failed_dib(10, 80, 39, list()) - 5
  )
  ...
  scan_model <- new_taper_model(
    id = "private.scan_failure", form = "test",
    dib = all_failed
  )
  scan <- merchandiser:::.numeric_inverse(scan_model, "dib", 10, 80, 5, list(), 1L)
  ...
  refinement_model <- new_taper_model(
    id = "private.refinement_scan_failure", form = "test",
    dib = refinement_failure
  )
  tracker$calls <- 0L
  target <- linear_dib(10, 80, 40.001, list())
  refinement <- merchandiser:::.numeric_inverse(
    refinement_model, "dib", 10, 80, target, list(),
    1L
  )
...
test_that("outside inversion covers missing and mixed bark ratios", {
  model <- new_taper_model(
    id = "private.mixed_bark", form = "test",
    dib = linear_dib, inputs = list(
      required = character(),
  ...
  missing <- height_at_dob(
    dbh = 10, ht = 80, dob = 5, model = "private.mixed_bark",
    spcd = .interface_spcd("private.mixed_bark")
  )
  expect_identical(missing$status, 53L)

  aux <- merchandiser:::.set_aux_caller_units(
    list(bark_ratio = c(NA_real_, 0.9)),
    "imperial"
  )
  mixed <- merchandiser:::.inverse_group(
    model, "dob", c(10, 10), c(80, 80), c(5, 5), aux,
    1:2
  )
  ...
  analytic_model <- new_taper_model(
    id = "private.mixed_bark_analytic", form = "test", dib = linear_dib,
    height_at_dib = inverse, inputs = list(
      required = character(), optional = "bark_ratio",
      pairs = list()
    ), bark_ratio = NA_real_
  )
  analytic <- merchandiser:::.inverse_group(analytic_model, "dob", c(10, 10), c(80, 80), c(
    5,
    5
  ), aux, 1:2)
...
test_that("invalid groups exit height, volume, and profile evaluation", {
  expect_identical(
    height_at_dib(
      dbh = 10, ht = 80, dib = 5, model = "missing.model",
      spcd = .interface_spcd("missing.model")
    )$status,
    50L
  )
  expect_identical(
    height_at_dib(
      dbh = 0, ht = 80, dib = 5, model = "demo.paraboloid",
      spcd = .interface_spcd("demo.paraboloid")
    )$status,
    2L
  )
  expect_identical(
    stem_volume(dbh = 10, ht = 80, model = "missing.model", spcd = .interface_spcd(
      "missing.model"
    ))$status,
    50L
  )
  expect_identical(
    stem_volume(dbh = 0, ht = 80, model = "demo.paraboloid", spcd = .interface_spcd(
      "demo.paraboloid"
    ))$status,
    2L
  )
  expect_identical(stem_profile(
    dbh = 10, ht = 80, model = "missing.model", spcd = .interface_spcd("missing.model"),
    tree_id = seq_along(rep_len(10, max(length(10), length(80))))
  )$status, 50L)
  expect_identical(stem_profile(
    dbh = 0, ht = 80, model = "demo.paraboloid", spcd = .interface_spcd("demo.paraboloid"),
    tree_id = seq_along(rep_len(0, max(length(0), length(80))))
  )$status, 2L)
})

test_that("quadrature covers a degenerate interval and a kernel status", {
  model <- get_taper_model("demo.paraboloid.r")
  zero <- merchandiser:::.gauss_legendre_integral(model, "dib", 10, 80, 20, 20, list(), 1L)
  ...
  failed <- stem_volume(
    dbh = 10, ht = 80, model = "private.quadrature_failure",
    spcd = .interface_spcd("private.quadrature_failure")
  )
  expect_identical(failed$status, 54L)
