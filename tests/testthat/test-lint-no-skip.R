# Parse every test and helper so comments and quoted examples cannot hide a forbidden call
# or be mistaken for one. Only a verified missing fixture directory permits a plain skip.
find_forbidden_skips <- function(code) {
  violations <- character()
  fixture_value <- quote(Sys.getenv("MERCHANDISER_FIXTURES", unset = Sys.getenv(
    "TREEVOLUME_FIXTURES",
    unset = ""
  )))
  walk <- function(node, bindings = list(), guarded = FALSE) {
    ...
    if (is.expression(node))
      node <- as.call(c(list(as.name("{")), as.list(node)))
    if (!is.call(node))
      return(bindings)
    head <- node[[1L]]
    name <- if (is.symbol(head))
      as.character(head) else ""
    if (is.call(head) && as.character(head[[1L]]) %in% c("::", ":::")) {
      name <- as.character(head[[3L]])
    }
    if (grepl("^skip($|_)", name)) {
      allowed <- name == "skip" && guarded
      if (!allowed)
        violations <<- c(violations, paste(deparse(node), collapse = " "))
    }
    if (name == "{") {
    ...
    if (name == "if") {
      ...
      for (variable in names(bindings)) {
        ...
        if (identical(bindings[[variable]], fixture_value)) {
          permitted <- permitted || identical(condition, substitute(
            !dir.exists(x),
            list(x = symbol)
          ))
        }
...
test_that("every test file skips only when its fixture directory is missing", {
  files <- list.files(testthat::test_path(),
  ...
  for (code in c(
    "skip(\"failure\")", "testthat::skip_if(TRUE)", "skip_if_not(FALSE)", "skip_on_ci()",
    "testthat::skip_on_cran()", "if (TRUE) skip(\"missing fixtures\")",
  ...
  expect_length(find_forbidden_skips(parse(text = c(
    "skip_if_not_installed(\"optional\")",
    "testthat::skip_if_not_installed(\"optional\")"
  ))), 2L)
  for (variable in "FIXTURES") {
    binding <- sprintf(
      paste0("guard <- Sys.getenv(\"MERCHANDISER_%s\", ",
             "unset = Sys.getenv(\"TREEVOLUME_%s\", unset = \"\"))"),
      variable, variable
    )
    condition <- if (variable == "FIXTURES")
      "!dir.exists(guard)" else "guard != \"true\""
    expect_identical(find_forbidden_skips(parse(text = c(binding, sprintf(
      "if (%s) testthat::skip(\"guarded\")",
      condition
    )))), character())
    expect_length(find_forbidden_skips(parse(text = c(binding, sprintf(
      "if (%s) TRUE else skip(\"wrong branch\")",
      condition
    )))), 1L)
  }
})
...
test_that("a checksum mismatch fails instead of skipping the oracle test", {
  ...
  environment(check) <- list2env(list(.mc_oracle_manifest = function(root, expected_files) {
...
test_that("a missing fixture directory still skips", {
  ...
  on.exit(
    {
      if (is.na(previous)) Sys.unsetenv("MERCHANDISER_FIXTURES") else Sys.setenv(
        MERCHANDISER_FIXTURES = previous
      )
    },
    add = TRUE
  )
  ...
  guards <- 0L
  files <- list.files(testthat::test_path(),
    pattern = "^test-(stem-nvel-|oracle-).*\\.R$",
    full.names = TRUE
  )
  for (file in files) {
    for (block in parse(file)) {
      if (!is.call(block) || !identical(block[[1L]], as.name("test_that")))
        next
      body <- block[[3L]]
      if (!is.call(body) || length(body) < 3L)
        next
      guard <- as.call(as.list(body)[1:3])
      if (!grepl("MERCHANDISER_FIXTURES", paste(deparse(guard), collapse = " ")))
        next
      guards <- guards + 1L
