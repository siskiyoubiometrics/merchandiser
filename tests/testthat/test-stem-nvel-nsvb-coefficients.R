...
test_that("NSVB species literals and public reference are complete", {
  path <- system.file("extdata", "nsvb_species_literals.csv",
  ...
  expect_identical(as.double(gsub("[dD]", "e", literals$literal)), literals$value)
  expect_true(all(literals$upstream_commit == "38548071d5aa652bb90c7f111f86b427f798a1c9"))

  expect_equal(nrow(species_reference), 2677L)
  expect_identical(names(species_reference), c(
...
test_that("NSVB Scribner literals retain executable provenance", {
  path <- system.file("extdata", "nsvb_scribner_coefficients.csv",
    package = "merchandiser",
    mustWork = TRUE
  )
  coefficients <- utils::read.csv(path, stringsAsFactors = FALSE)
  ...
  expect_identical(as.double(gsub("[dD]", "e", coefficients$literal)), coefficients$value)
})
...
test_that("county division lookup has pinned Forest Service provenance", {
  ...
  provenance_path <- system.file("extdata", "nsvb_county_divisions_provenance.csv",
    package = "merchandiser",
    mustWork = TRUE
  )
