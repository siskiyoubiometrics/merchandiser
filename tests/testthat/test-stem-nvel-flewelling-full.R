...
full_flewelling_exclusions <- list(
  flewelling_2pt = list(
    diameter = c(nonzero_errflag = 7968L, upper_at_or_above_total = 0L),
    height = c(
      nonzero_errflag = 6915L,
      upper_at_or_above_total = 0L,
      sf_hs_bark_height = 11525L,
  ...
  flewelling_3pt = list(
    ...
    height = c(
      nonzero_errflag = 0L,
      upper_at_or_above_total = 4593L,
      sf_hs_bark_height = 0L,
      sf_hs_conditioned_root = 23082L,
      at_or_below_contract_stump = 0L
    )
  )
)

full_r10r4_exclusions <- list(
  r10_taper = list(
    diameter = c(missing_bru_dispatch = 14170L),
    height = c(
...
test_that("full Flewelling fixtures agree when supplied", {
  ...
  for (family in c("flewelling_2pt", "flewelling_3pt")) {
    ...
    actual <- .check_flewelling_fixture(
      double_path, "double", full_flewelling_exclusions[[family]],
      record = TRUE
    )
    .check_flewelling_fixture(
      single_path, "single", full_flewelling_exclusions[[family]], actual,
      record = TRUE,
      expected_compat_precision = if (family == "flewelling_2pt") 5L else 8L
    )
  }
  for (family in c("r10_taper", "r4_driver")) {
    ...
    expect_true(file.exists(double_path), info = double_path)
    expect_true(file.exists(single_path), info = single_path)
    actual <- .check_r10r4_fixture(
      double_path, family, "double", full_r10r4_exclusions[[family]],
      record = TRUE
    )
    ...
    .check_r10r4_fixture(
      single_path, family, "single", single_exclusions, actual,
      record = TRUE
    )
  }
})
