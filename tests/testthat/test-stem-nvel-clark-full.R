...
test_that("full Clark fixtures agree when supplied", {
  ...
  for (precision in c("double", "single")) {
    for (family in names(expected_identifiers)) {
      ...
      expect_equal(length(metrics$identifiers), expected_identifiers[[family]],
        info = paste(
          family,
          precision
        )
      )
      expected_overlap <- if (identical(family, "clark_r8"))
        4L else 0L
      ...
      .gate1_record_summary(
        family, "DIB", precision, dib$compared, dib$compared - dib$failures,
        dib$max_relative, tv_tolerance[[paste0("diameter_", precision, "_rel")]], list(
      ...
      if (expected_overlap) {
        outside_overlap <- height$compared + height$excluded_nonzero_errflag +
          height$excluded_nonfinite +
          height$excluded_below_stump
        .gate1_record_summary(family, "height at DIB", precision,
          metrics$compat_overlap$compared,
          metrics$compat_overlap$within, metrics$compat_overlap$max_relative,
          tv_tolerance[[paste0(
            "height_",
            precision, "_rel"
      ...
      .gate1_record_summary(
        ...
        height$compared -
          height$failures, height$max_relative, tv_tolerance[[paste0(
          "height_", precision,
          "_rel"
        )]], list(
