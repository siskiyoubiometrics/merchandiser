...
test_that("the compiled provider preserves basal-area units", {
  ...
  aux <- list(site_index = 100 * 0.3048, basal_area = 120 * area_factor, bark_ratio = 0.9)
  compiled <- merchandiser:::mc_provider_query(
    session$pointer, 10 * 2.54, 80 * 0.3048, 1L,
    aux, 1L, 40 * 0.3048, 2L
  )
  public <- .interface_metric_result(merchandiser::dib(
    dbh = 10 * 2.54 / 2.54, ht = 80 * 0.3048 / 0.3048,
    h = 40 * 0.3048 / 0.3048, model = model, site_index = aux$site_index,
    basal_area = aux$basal_area,
    bark_ratio = aux$bark_ratio, spcd = .interface_spcd(model)
  ), 2.54)
...
test_that("the compiled provider accepts an empty auxiliary list", {
  ...
  crossings <- merchandiser:::mc_provider_crossings(
    session$pointer, 12, 80, 1L, list(), 4,
    0L, 1, 80, 1L
  )
...
test_that("R fallback returns every discovered diameter crossing", {
  ...
  if (merchandiser::has_taper_model(model_id))
    merchandiser::unregister_taper_model(model_id)
  on.exit(
    {
      gc()
      if (merchandiser::has_taper_model(
        model_id
      )) merchandiser::unregister_taper_model(model_id)
    },
    add = TRUE
  ...
  model <- merchandiser::new_taper_model(
    id = model_id, form = "test", dib = profile, stump_ht = 0,
    bark_ratio = 1, source = "merchandiser all-crossing test"
  )
  merchandiser::register_taper_model(model)
  roots <- merchandiser:::.mc_r_crossing_basis(list(
    dbh = 12, ht = 20, model = model_id, spcd = 202
  ), target = 10, lower = 0, upper = 10, basis = "ib")[[1L]]
  expect_identical(attr(roots, "status"), 0L)
  expect_equal(as.double(roots), c(0, 1, 2, 3, 4, 4.5), tolerance = 1e-04)
})

test_that("crossing and profile passes do not grow by tree or log", {
  specification <- product(
    product = "log", min_length = 8, max_length = 16,
    trim = 0.5, min_sed = 3, volume_unit = "cubic"
  )
  ...
  testthat::local_mocked_bindings(.mc_all_crossings = function(...) {
  ...
  merchandise(1:3, rep(12, 3), rep(60, 3), rep(202, 3),
    specification,
...
test_that("the compiled provider converts green weight in both unit systems", {
  volume <- c(1, 2)
  imperial <- merchandiser:::mc_provider_green_weight(volume, c(202L, 202L), c(0L, 1L), 1L)
  metric <- merchandiser:::mc_provider_green_weight(
    volume * 0.028316846592, c(202L, 202L),
    c(0L, 1L), 2L
  )
  ...
  expect_error(
    merchandiser:::mc_provider_green_weight(volume, 202L, c(0L, 1L), 1L),
    "inconsistent sizes"
  )
})
