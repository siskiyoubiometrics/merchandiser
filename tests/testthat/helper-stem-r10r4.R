...
# Measured values proposed for adoption in helper-tolerance.R.
# They are deliberately rounded upward from the maxima in REPORT_R10R4.md.
.r10r4_tolerance <- list(
  diameter_double_rel = 1e-12,
  height_double_rel = 1e-10,
  diameter_single_rel = 5e-6,
  height_single_rel = 1e-3
)
...
.r10r4_evaluate_height <- function(data, include) {
  ...
  if (!length(selected)) return(data.frame(value = value, status = status))
  ...
  for (start in starts) {
    ...
    result <- merchandiser::height_at_dib(
      data$DBHOB[rows], data$HTTOT[rows], data$STEMDIB[rows],
      data$VOLEQ[rows], status = TRUE
    )
...
.check_r10r4_fixture <- function(path, family, precision,
                                 expected_exclusions, actual = NULL,
                                 record = FALSE) {
  fixture <- .read_r10r4_fixture(path)
  input_columns <- c(
    "ROW_ID", "CALL_KIND", "VOLEQ", "DBHOB", "HTTOT", "HTUP",
    "STEMDIB", "CALCDIA_REQUESTED", "HT2TOPD_REQUESTED"
  )
  inputs <- fixture[, input_columns, drop = FALSE]
  if (!is.null(actual)) expect_identical(inputs, actual$inputs)

  diameter <- fixture[
    fixture$CALL_KIND == "PROFILE_PROBE" &
      fixture$CALCDIA_REQUESTED == "Y" & fixture$ERRFLAG == 0L &
      fixture$CALCDIA_ERRFLAG == 0L, , drop = FALSE
  ]
  if (identical(family, "r10_taper")) {
    diameter_masks <- list(
      missing_bru_dispatch = substr(diameter$VOLEQ, 4L, 6L) == "BRU"
    )
  } else {
    diameter_masks <- list(nonzero_errflag = rep(FALSE, nrow(diameter)))
  }
  diameter_excluded <- .expect_r10r4_exclusion_counts(
    diameter_masks, expected_exclusions$diameter
  )
  inside <- merchandiser::dib(
  ...
  inverse <- fixture[
    fixture$CALL_KIND == "PROFILE_PROBE" &
      fixture$HT2TOPD_REQUESTED == "Y" & fixture$ERRFLAG == 0L &
      fixture$HT2TOPD_ERRFLAG == 0L, , drop = FALSE
  ]
  if (identical(family, "r10_taper")) {
    bru <- substr(inverse$VOLEQ, 4L, 6L) == "BRU"
    inverse_masks <- list(
      missing_bru_dispatch = bru,
      unsupported_dispatch_species = !bru &
        substr(inverse$VOLEQ, 8L, 10L) %in% c("202", "260")
    )
  } else {
    inverse_masks <- list(
      zero_outside_contract = is.finite(inverse$STEMHT) &
        inverse$STEMHT == 0,
      nonfinite_oracle = !is.finite(inverse$STEMHT)
    )
  }
  inverse_excluded <- .expect_r10r4_exclusion_counts(
    inverse_masks, expected_exclusions$height
  )
  ...
  .expect_r10r4_relative(
    ...
    .r10r4_tolerance[[paste0("height_", precision, "_rel")]]
  )
  if (identical(family, "r10_taper")) {
    source_omission <- inverse_masks$unsupported_dispatch_species
    compat_height <- .with_treevolume_compat(
      "nvel", .r10r4_evaluate_height(inverse, source_omission)
    )
    expect_true(all(compat_height$status[source_omission] == 0L))
    expect_identical(
      compat_height$value[source_omission], inverse$STEMHT[source_omission]
    )
  }
  if (record) {
    inverse_requested <- fixture$CALL_KIND == "PROFILE_PROBE" &
    ...
    .gate1_record_values(
      ...
      good_height,
      .r10r4_tolerance[[paste0("height_", precision, "_rel")]], masks
    )
    if (identical(family, "r10_taper")) {
      .gate1_record_values(
        family, "height at DIB", precision,
        compat_height$value, inverse$STEMHT, source_omission,
        0,
        list(
