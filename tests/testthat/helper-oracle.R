.mc_oracle_columns <- c(
  ...
  "VOL10", paste0("LOGLEN", 1:20), paste0("LOGDIA_", 2:21, "_1"), paste0(
    "LOGDIA_",
    2:21, "_2"
  ),
  paste0("LOGVOL_1_", 1:20)
)

.mc_oracle_products <- function() {
  specification <- .mc_cpp_products(products(
    product("scribner_decimal",
      min_length = 16, max_length = 16,
      min_sed = 0, volume_unit = "scribner", split_scale = TRUE
    ), product("scribner_factor",
      min_length = 16, max_length = 16, min_sed = 0, volume_unit = "scribner",
      split_scale = TRUE
    ),
    product("international",
      min_length = 16, max_length = 16, min_sed = 0,
      volume_unit = "international"
    )
  ))
  # Exercise the unchanged source scaling rules directly.
  specification$scale_rule <- c(0L, 3L, 6L)
  specification
}
...
.mc_oracle_native_scale <- function(products, product, diameter, length) {
  ...
  if (length(product) != length(length)) {
    stop("Native oracle product indices have an inconsistent size.", call. = FALSE)
  }
  mc_nvel_log_scale_cpp(
    products, as.integer(product), as.double(diameter),
    as.double(length)
  )
}
...
.mc_oracle_manifest <- function(root, expected_files) {
  ...
  manifest_root <- if (basename(root) == "full")
    dirname(root) else root
  ...
  if (any(!file.exists(paths))) {
    stop("Oracle fixture inventory is incomplete. Missing: ", paste(
      basename(
...
.mc_oracle_empty_metrics <- function(family, precision) {
  list(
    family = family, precision = precision, source_rows_scanned = 0, rows_compared = 0,
    source_logs = 0, included_logs = 0, scribner_compared = 0, scribner_source_logs = 0,
    scribner_within_tolerance = 0, international_compared = 0,
    international_within_tolerance = 0,
    excluded_not_volumelibrary = 0, excluded_oracle_error = 0, excluded_no_log_table = 0,
    excluded_missing_log_fields = 0, excluded_f32_unpaired_logs = 0,
    excluded_f32_incomplete_logs = 0,
    excluded_scribner_non_split_logs = 0, excluded_international_family = 0,
    excluded_international_incomplete = 0,
...
.mc_oracle_process_logs <- function(chunk, family, products, metrics) {
  volume_call <- !is.na(chunk$CALL_KIND) & chunk$CALL_KIND == "VOLUMELIBRARY"
  successful <- volume_call & !is.na(chunk$ERRFLAG) & chunk$ERRFLAG == 0
  has_logs <- successful & is.finite(chunk$NOLOGP) & chunk$NOLOGP > 0
  metrics <- .mc_oracle_add(metrics, "excluded_not_volumelibrary", sum(!volume_call))
  metrics <- .mc_oracle_add(metrics, "excluded_oracle_error", sum(volume_call &
                                                                    !successful))
  metrics <- .mc_oracle_add(metrics, "excluded_no_log_table", sum(successful & !has_logs))
  if (!any(has_logs))
    return(metrics)
  ...
  slot <- matrix(rep(seq_len(20L), each = nrow(rows)), nrow = nrow(rows)) <= count
  valid <- slot & is.finite(lengths) & lengths > 0 & is.finite(diameters) & diameters > 0 &
    is.finite(observed)
  ...
  metrics <- .mc_oracle_add(metrics, "excluded_missing_log_fields", source_logs -
                              included_logs)

  if (identical(family, "clark_r9")) {
    metrics <- .mc_oracle_add(metrics, "excluded_scribner_non_split_logs", included_logs)
  }
  if (identical(family, "flewelling_3pt")) {
    ...
    if (any(ordinary)) {
      expected <- .mc_oracle_native_scale(
        products, 1L, diameters[ordinary],
        lengths[ordinary]
      )
      actual <- observed[ordinary]
      metrics <- .mc_oracle_add(metrics, "scribner_compared", length(expected))
      metrics <- .mc_oracle_add(metrics, "scribner_source_logs", length(expected))
      metrics <- .mc_oracle_add(metrics, "scribner_within_tolerance", sum(
        actual == expected
      ))
    }
    for (top in seq.int(2L, 20L, by = 2L)) {
      ...
      usable <- expected_pair & valid[, top - 1L] & valid[, top] & is.finite(raw)
      incomplete <- expected_pair & !usable
      metrics <- .mc_oracle_add(metrics, "excluded_f32_incomplete_logs", sum(valid[
        incomplete,
        top - 1L
      ]) + sum(valid[incomplete, top]))
      if (any(usable)) {
        pair_length <- lengths[usable, top - 1L] + lengths[usable, top]
        expected <- .mc_oracle_native_scale(products, 1L, trunc(raw[usable]), pair_length)
        actual <- observed[usable, top - 1L] + observed[usable, top]
        metrics <- .mc_oracle_add(metrics, "scribner_compared", length(expected))
        metrics <- .mc_oracle_add(metrics, "scribner_source_logs", 2L * length(expected))
        metrics <- .mc_oracle_add(metrics, "scribner_within_tolerance", sum(actual ==
                                                                              expected))
      }
    }
    odd <- which(special & count %% 2L == 1L)
    unpaired <- if (length(odd))
      valid[cbind(odd, count[odd])] else logical()
    metrics <- .mc_oracle_add(metrics, "excluded_f32_unpaired_logs", sum(unpaired))
    ...
    expected <- .mc_oracle_native_scale(products, product, diameters[valid], lengths[valid])
    actual <- observed[valid]
    metrics <- .mc_oracle_add(metrics, "scribner_compared", length(expected))
    metrics <- .mc_oracle_add(metrics, "scribner_source_logs", length(expected))
    metrics <- .mc_oracle_add(metrics, "scribner_within_tolerance", sum(actual == expected))
  }

  if (startsWith(family, "clark_")) {
    metrics <- .mc_oracle_add(metrics, "excluded_international_family", nrow(rows))
  } else {
    complete <- rowSums(valid) == count & is.finite(rows$VOL10)
    metrics <- .mc_oracle_add(metrics, "excluded_international_incomplete", sum(!complete))
    if (any(complete)) {
      ...
      for (at in seq_along(indices)) {
        ...
        expected[at] <- sum(.mc_oracle_native_scale(
          products, 3L, diameters[row, logs],
          lengths[row, logs]
        ))
        actual[at] <- rows$VOL10[row]
      }
      metrics <- .mc_oracle_add(metrics, "international_compared", length(expected))
      metrics <- .mc_oracle_add(metrics, "international_within_tolerance", sum(actual ==
                                                                                 expected))
    }
  }
  metrics
...
.mc_oracle_stream_file <- function(path, family, precision, products, chunk_size = 50000L) {
  if (!requireNamespace("data.table", quietly = TRUE)) {
    stop("The data.table package is required for full oracle streaming.", call. = FALSE)
  }
  ...
  repeat {
    ...
    chunk <- data.table::fread(
      text = paste(c(header, lines), collapse = "\n"), select = .mc_oracle_columns,
      na.strings = c("", "NA", "NaN", "-nan"), showProgress = FALSE, data.table = FALSE
    )
    metrics <- .mc_oracle_add(metrics, "source_rows_scanned", nrow(chunk))
    metrics <- .mc_oracle_add(metrics, "invalid_build_rows", sum(is.na(
      chunk$BUILD
    ) | chunk$BUILD !=
      precision))
...
mc_full_oracle_agreement <- function(root) {
  ...
  if (any(manifest$checksum_mismatch)) {
    ...
    stop(paste("Oracle fixture checksum mismatch:", paste(mismatch,
           collapse = ", "
         )), call. = FALSE)
  }
  ...
  for (at in seq_along(expected_files)) {
    ...
    output[[at]] <- .mc_oracle_stream_file(manifest$paths[at], family, precision, products)
  }
...
.mc_oracle_public_case <- function(root) {
  ...
  full <- if (basename(root) == "full")
    root else file.path(root, "full")
  path <- file.path(full, "flewelling_2pt.double.csv.gz")
  connection <- gzfile(path, "rt")
  on.exit(close(connection), add = TRUE)
  header <- readLines(connection, n = 1L, warn = FALSE)
  columns <- c(
    "ROW_ID", "CALL_KIND", "REGN", "FIASPCD", "VOLEQ", "DBHOB", "HTTOT", "MTOPP",
    "MTOPS", "STUMP", "MRULEMOD", "ERRFLAG", "NOLOGP", "LOGLEN1", "BOLHT2", "LOGVOL_1_1",
    "VOL10"
  )
  repeat {
    ...
    selected <- with(chunk, CALL_KIND == "VOLUMELIBRARY" & REGN == 1L & MRULEMOD == "N" &
                       ERRFLAG == 0L &
                       NOLOGP == 1 & is.finite(LOGLEN1) & LOGLEN1 > 0 & is.finite(BOLHT2) &
                       is.finite(LOGVOL_1_1) & is.finite(VOL10))
    if (any(selected))
      return(chunk[which(selected)[1L], , drop = FALSE])
  }
