test_that("Flewelling coefficient literals round trip with provenance", {
  path <- system.file(
    "extdata", "flewelling_coefficients.csv", package = "merchandiser",
    mustWork = TRUE
  )
  ...
  expect_equal(
    length(unique(paste(coefficients$source_file,
                        coefficients$source_line_start))),
    237L
  )
  expect_setequal(
  ...
  expect_true(
    all(coefficients$upstream_commit ==
