test_that("new_taper_model constructs the versioned class", {
  model <- new_taper_model("test.constructor", "user", linear_dib)

  expect_s3_class(model, "taper_model")
  expect_identical(model$class_version, "0.3")
  expect_identical(attr(model, "oracle_verified"), NA)
  expect_identical(model$data, list())
  expect_identical(model$species, integer())
  expect_named(model$kernel, c(
    "type", "key", "has_dob", "has_inverse", "has_integral"
  ))
...
test_that("oracle verification metadata is validated and printed", {
  ...
  first <- dib(12, 80, c(1, 20, 40, 79), model$id, status = TRUE)
  second <- dib(12, 80, c(1, 20, 40, 79), model$id, status = TRUE)
...
test_that("model validation rejects malformed metadata and callbacks", {
  expect_error(new_taper_model("bad", "user", function(x) x), "formals")
  expect_error(
    new_taper_model("bad", "user", linear_dib, units = "yards"),
    "units"
  )
  numeric_species <- new_taper_model(
    "test.numeric_species", "user", linear_dib, species = c(122, 202)
  )
  expect_identical(numeric_species$species, c(122L, 202L))
  expect_identical(
    new_taper_model("test.unrestricted", "user", linear_dib)$species,
    integer()
  )
  numeric_species$species <- c(202, 263)
  expect_identical(
    validate_taper_model(numeric_species)$species,
    c(202L, 263L)
  )
  expect_error(
    new_taper_model("bad", "user", linear_dib, species = 122.5),
    "whole-number"
  )
  expect_error(
    new_taper_model("bad", "user", linear_dib, species = 0),
    "positive whole-number"
  )
  expect_error(
    new_taper_model("bad", "user", linear_dib, species = -122),
    "positive whole-number"
  )
  invalid_species <- list(
    "202", matrix(202, nrow = 1L), NA_real_, Inf,
    as.double(.Machine$integer.max) + 1
  )
  for (species in invalid_species) {
    expect_error(
      new_taper_model("bad", "user", linear_dib, species = species),
      "species"
    )
  }
  ...
  numeric_species$species <- c(122, 202)
  merchandiser:::.register_package_model(numeric_species)
  expect_identical(
    get_taper_model("test.package_numeric_species")$species,
    c(122L, 202L)
  )
  expect_error(
    new_taper_model("bad", "user", linear_dib, bark_ratio = 2),
    "bark_ratio"
  )
  increasing <- function(dbh, ht, h, aux) as.double(dbh + h)
  expect_error(new_taper_model("bad", "user", increasing), "monotone")
  integer_return <- function(dbh, ht, h, aux) rep(1L, length(dbh))
  expect_error(new_taper_model("bad", "user", integer_return), "doubles")
})

test_that("NVEL-like private identifiers produce the specified warning", {
  expect_warning(
    new_taper_model("1234567890", "user", linear_dib),
    "resembles an NVEL"
  )
})
...
test_that("input declarations and pairs are validated", {
  expect_error(
    new_taper_model(
      "bad.inputs", "user", linear_dib,
      inputs = list(optional = character(), required = character(), pairs = list())
    ),
    "in that order"
  )
  expect_error(
    new_taper_model(
      "bad.pair", "user", linear_dib,
      inputs = list(
        required = character(), optional = "upper_ht1",
  ...
  upper_bark_model <- new_taper_model(
    "test.upper_bark", "user", upper_bark_dib,
...
test_that("validation rejects and rolls back callback side effects", {
  ...
  expect_error(
    new_taper_model("bad.global", "user", global_writer),
    "global environment"
  )
  ...
  expect_error(
    new_taper_model("bad.global_rewrite", "user", global_rewriter),
    "global environment"
  )
  ...
  expect_error(
    new_taper_model("bad.global_remove", "user", global_remover),
    "global environment"
  )
  expect_identical(get(global_name, envir = .GlobalEnv), 1L)
  ...
  expect_error(
    new_taper_model("bad.options", "user", option_writer), "changed options"
  )
  ...
  expect_error(
    new_taper_model("bad.option_add", "user", option_adder), "changed options"
  )
  ...
  expect_error(
    new_taper_model("bad.rng", "user", rng_writer), "RNG state"
  )
  expect_identical(
  ...
  expect_error(
    new_taper_model("bad.warning", "user", warning_callback),
  ...
  expect_error(
    new_taper_model("bad.message", "user", message_callback),
  ...
  expect_error(
    new_taper_model("bad.error", "user", error_callback),
    "raised a condition.*probe error"
  )

  intruder <- new_taper_model("private.validation_intruder", "test", linear_dib)
  registry_writer <- function(dbh, ht, h, aux) {
    register_taper_model(intruder)
    as.double(dbh * (1 - h / ht))
  }
  expect_error(
    new_taper_model("bad.registry", "user", registry_writer),
...
test_that("optional analytic callbacks are validated on probe vectors", {
  ...
  model <- new_taper_model(
    "test.callbacks", "user", linear_dib,
    dob = outside, height_at_dib = inverse, volume = volume
  )
  ...
  expect_error(
    new_taper_model("bad.dob", "user", linear_dib, dob = bad_dob),
    "dob must return"
  )
})
...
test_that("validate_taper_model rejects damaged representations", {
  ...
  model <- new_taper_model("test.damage", "user", linear_dib)
  model$notes <- NULL
  expect_error(validate_taper_model(model), "fields")

  model <- new_taper_model("test.flags", "user", linear_dib)
  model$kernel$has_dob <- TRUE
  expect_error(validate_taper_model(model), "capability flags")

  model <- new_taper_model("test.mutable-data", "user", linear_dib)
