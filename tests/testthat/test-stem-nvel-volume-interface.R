test_that("internal NVEL volume record has frozen shape and statuses", {
  result <- merchandiser:::nvel_volume(c(12, 12), c(80, 80), c("300FW2W202", "900DVEE122"),
    id = c("profile", "direct"), status = TRUE
  )
  expect_identical(result$id, c("profile", "direct"))
  expect_identical(names(result)[2:19], c(
...
test_that("merchant rules validate NVEL domains", {
  ...
  expect_named(rules, c(
    "even_or_odd", "option", "maximum_length", "minimum_length", "minimum_top_length",
    "merchantable_length", "primary_top", "secondary_top", "stump", "trim", "bark_ratio",
    "minimum_board_foot_dbh", "scribner", "prod", "ht_type", "live", "ctype",
    "cull", "forest",
    "district"
  ))
...
test_that("regional merchant defaults and cull routing follow NVEL", {
  ...
  region1_values$prod <- c("01", "08", "08", "01")
  region1_values$primary_top <- c(NA, NA, NA, 4)
  region1_values$secondary_top <- c(NA, NA, NA, 6)
  region1 <- merchandiser:::.nvel_rule_defaults(region1_values, c(
    "100JB2W202", "100JB2W202",
    "300FW2W202", "100JB2W202"
  ...
  nsvb <- "NVBM240202"
  sound <- merchandiser:::nvel_volume(12, 80, nsvb,
    rules = merchandiser:::nvel_rules(cull = 0),
    status = TRUE
  )
  culled <- merchandiser:::nvel_volume(12, 80, nsvb,
    rules = merchandiser:::nvel_rules(cull = 10),
    status = TRUE
  )
  expect_equal(as.numeric(culled[c(1L, 14L, 15L)]), as.numeric(sound[c(
    1L, 14L,
    15L
  )]) * 0.9,
  tolerance = 1e-12
  ...
  profile <- "300FW2W202"
  unculled_profile <- merchandiser:::nvel_volume(12, 80, profile,
    rules = merchandiser:::nvel_rules(cull = 0),
    status = TRUE
  )
  culled_profile <- merchandiser:::nvel_volume(12, 80, profile,
    rules = merchandiser:::nvel_rules(cull = 10),
    status = TRUE
  )
  expect_equal(culled_profile[c(1L, 14L)], unculled_profile[c(1L, 14L)], tolerance = 0)
})
