...
.gate1_exclusions <- function(masks) {
  if (!length(masks)) return("none")
  counts <- vapply(masks, function(mask) sum(mask, na.rm = TRUE), numeric(1L))
  counts <- counts[counts > 0]
  if (!length(counts)) return("none")
  paste(paste(names(counts), format(counts, scientific = FALSE)), collapse = ", ")
}

.gate1_record_values <- function(family, output, precision, actual, expected,
                                 include, tolerance, masks = list(),
  ...
  relative <- abs(actual - expected) /
    pmax(abs(expected), denominator_floor)
  compared <- sum(include)
  within <- sum(include & is.finite(relative) & relative <= tolerance)
  maximum <- if (compared) max(relative[include], na.rm = TRUE) else NA_real_
  .gate1_metrics$records[[length(.gate1_metrics$records) + 1L]] <- data.frame(
    ...
    rows_within_tolerance = as.numeric(within),
    max_relative_difference = maximum,
    tolerance = tolerance,
    exclusions = .gate1_exclusions(masks),
    stringsAsFactors = FALSE
  )
  invisible(NULL)
}

.gate1_record_summary <- function(family, output, precision, compared, within,
                                  maximum, tolerance, masks = list(),
                                  compat = "port") {
  if (!nzchar(Sys.getenv("TREEVOLUME_GATE1_RESULTS", unset = ""))) {
    return(invisible(NULL))
  }
