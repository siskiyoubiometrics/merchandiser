.flewelling_columns <- c(
  ...
  "MTOPP", "STUMP", "UPSHT1", "UPSHT2", "UPSD1", "UPSD2", "HTUP",
  "STEMDIB", "CALCDIA_REQUESTED", "HT2TOPD_REQUESTED", "MRULEMOD",
  "ERRFLAG", "CALCDIA_ERRFLAG", "HT2TOPD_ERRFLAG",
  "DIB", "DOB", "STEMHT"
)
...
.fw_invalid_conditioning <- function(data) {
  ...
  invalid <- form %in% c("FW3", "F33") & (
    data$UPSHT1 >= data$HTTOT |
      (data$UPSHT2 > 0 & data$UPSHT2 >= data$HTTOT)
  )
...
.fw_other_group <- function(data) {
  ...
  (region == "2" & species %in% c(122L, 108L, 202L, 15L, 746L)) |
    (region == "4" & geosub == "07" & species %in% c(93L, 122L)) |
    (region == "3" & geosub == "00" & species %in% c(122L, 202L)) |
    (region == "3" & geosub == "01" &
       species %in% c(122L, 108L, 202L, 15L))
}
...
.expect_flewelling_status <- function(actual, expected,
                                      allow_not_unique = FALSE) {
  failed <- expected != 0L
  expect_identical(actual[failed], expected[failed])
  successful <- !failed
  allowed <- if (allow_not_unique) c(0L, 102L) else 0L
...
.flewelling_evaluate <- function(data, operation) {
  ...
  second <- three_point & !is.na(data$UPSHT2) & data$UPSHT2 > 0 &
    !is.na(data$UPSD2) & data$UPSD2 > 0
  groups <- split(seq_len(nrow(data)), interaction(three_point, second))
  batch_size <- if (identical(operation, "height")) 128L else 4096L
  for (group_rows in groups) {
    starts <- seq.int(1L, length(group_rows), by = batch_size)
    for (start in starts) {
      rows <- group_rows[seq.int(
        start, min(start + batch_size - 1L, length(group_rows))
      )]
      arguments <- list(
      ...
      if (any(three_point[rows])) {
        f32 <- form[rows] == "F32"
        arguments$upper_ht1 <- ifelse(
          f32 & (is.na(data$UPSHT1[rows]) | data$UPSHT1[rows] <= 0),
          1, data$UPSHT1[rows]
        )
        arguments$upper_d1 <- ifelse(
          f32 & (is.na(data$UPSD1[rows]) | data$UPSD1[rows] <= 0),
          1, data$UPSD1[rows]
        )
        if (any(second[rows])) {
...
.check_flewelling_fixture <- function(path, precision, expected_exclusions,
                                      actual = NULL, record = FALSE,
                                      expected_compat_precision = 0L) {
  fixture <- .read_flewelling_fixture(path)
  input_columns <- c(
    "ROW_ID", "FAMILY", "CALL_KIND", "VOLEQ", "DBHOB", "HTTOT",
    "UPSHT1", "UPSHT2", "UPSD1", "UPSD2", "HTUP", "STEMDIB",
    "CALCDIA_REQUESTED", "HT2TOPD_REQUESTED"
  )
  inputs <- fixture[, input_columns, drop = FALSE]
  if (!is.null(actual)) {
    # The full single and double corpora use identical requests. Reusing the
    # package evaluation avoids repeating the expensive inverse grid while
    # still comparing every row with both oracle builds.
    expect_identical(inputs, actual$inputs)
  }

  diameter_rows <- fixture$CALL_KIND == "PROFILE_PROBE" &
    fixture$CALCDIA_REQUESTED == "Y"
  diameter <- fixture[diameter_rows, , drop = FALSE]
  expected_status <- .flewelling_expected_status(
  ...
  for (diameter_result in list(inside, outside)) {
    ...
    .expect_flewelling_status(
      diameter_result$status[!excluded], expected_status[!excluded]
    )
  }
  good <- !excluded
  .expect_flewelling_relative(
    inside$value[good], diameter$DIB[good],
    tv_tolerance[[paste0("diameter_", precision, "_rel")]]
  )
  .expect_flewelling_relative(
    outside$value[good], diameter$DOB[good],
    tv_tolerance[[paste0("diameter_", precision, "_rel")]]
  )
  if (record) {
    family <- unique(fixture$FAMILY)
    masks <- list(
      nonzero_errflag = failed,
      upper_at_or_above_total = invalid
    )
    tolerance <- tv_tolerance[[paste0("diameter_", precision, "_rel")]]
    .gate1_record_values(
      family, "DIB", precision, inside$value, diameter$DIB,
      good, tolerance, masks
    )
    .gate1_record_values(
      family, "DOB", precision, outside$value, diameter$DOB,
      good, tolerance, masks
    )
  }

  height_rows <- fixture$CALL_KIND == "PROFILE_PROBE" &
  ...
  compat_height <- if (is.null(actual)) {
    .with_treevolume_compat(
      "nvel", .flewelling_evaluate(inverse, "height")
    )
  } else {
  ...
  # sf_hs.f lines 109 through 121 pass HI2, rather than the trial height H,
  # to BRK_UP for the JSP 22 to 29 inside-bark conversion.
  fixed_bark_height <- !failed & !invalid & form == "FW2" &
    .fw_other_group(inverse)
  # sf_hs.f uses a Newton search that can select a lower or approximate root
  # for conditioned profiles. The package contract requires the highest root.
  conditioned_sf_hs <- !failed & !invalid & !fixed_bark_height &
    form %in% c("FW3", "F33")
  ...
  outside_contract_interval <- !failed & !invalid & !fixed_bark_height &
    !conditioned_sf_hs & is.finite(inverse$STEMHT) & inverse$STEMHT <= 1
  excluded <- .expect_fw_exclusion_counts(
    list(
      nonzero_errflag = failed,
      upper_at_or_above_total = invalid,
  ...
  expect_identical(
    height$status[outside_contract_interval],
    rep(101L, sum(outside_contract_interval))
  )
  ...
  root <- (fixed_bark_height | conditioned_sf_hs) &
    height$status %in% c(0L, 102L)
  ...
  if (any(root)) {
    ...
    expect_identical(
      diameter_at_height$status, integer(nrow(diameter_at_height))
    )
    .expect_flewelling_relative(
  ...
  .expect_flewelling_status(
    height$status[!excluded], expected_status[!excluded],
  ...
  compatibility_tolerance <-
    tv_tolerance[[paste0("height_", precision, "_rel")]]
  oracle_precision_difference <- abs(reference_compat - inverse$STEMHT) /
    pmax(abs(inverse$STEMHT), 1e-12)
  compat_precision_boundary <- compat_affected & precision == "single" &
    is.finite(oracle_precision_difference) &
    oracle_precision_difference > compatibility_tolerance
  expect_identical(
    sum(compat_precision_boundary), expected_compat_precision
  )
  compat_good <- compat_affected & is.finite(inverse$STEMHT) &
    !compat_precision_boundary
  ...
  if (record) {
    ...
    .gate1_record_values(
      ...
      height$value, inverse$STEMHT, good,
      tv_tolerance[[paste0("height_", precision, "_rel")]], masks
    )
    .gate1_record_values(
      unique(fixture$FAMILY), "height at DIB", precision,
      compat_height$value, inverse$STEMHT, compat_good,
      tv_tolerance[[paste0("height_", precision, "_rel")]],
      list(
        outside_compatibility_ruling = !fixed_bark_height &
          !conditioned_sf_hs,
        nonfinite_oracle = (fixed_bark_height | conditioned_sf_hs) &
          !is.finite(inverse$STEMHT),
  ...
  invisible(list(
    inputs = inputs, inside = inside, outside = outside,
    height = height, compat_height = compat_height,
