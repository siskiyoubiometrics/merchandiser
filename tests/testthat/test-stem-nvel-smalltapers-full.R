...
# supplied outside the package checkout through TREEVOLUME_FIXTURES.
test_that("full regional small-taper fixtures agree when supplied", {
  ...
  if (!dir.exists(root)) skip("MERCHANDISER_FIXTURES directory is missing")
  for (precision in c("double", "single")) {
    for (family in c(
      ...
      .check_smalltaper_fixture(
        .smalltaper_fixture_path(root, family, precision), precision,
        record = TRUE,
        expected_source_domain = if (identical(family, "r1_taper")) {
          1986L
        } else {
          0L
        },
        expected_below_contract = switch(
          family,
          r1_taper = 12L,
          blm_taper = 3441L,
          behre_taper = 214L,
