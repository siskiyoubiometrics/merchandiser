.nsvb_biomass_columns <- c(paste0("dry_", c(
  "aboveground_no_foliage", "stem_wood", "stem_bark", "stump_wood",
  "stump_bark", "saw_wood", "saw_bark", "topwood_wood", "topwood_bark", "tip_wood",
  "tip_bark",
  "branches", "foliage", "top_and_limb"
)), "carbon", paste0("green_", c(
  "agb_no_foliage", "stem_wood",
  "stem_bark", "stump_wood", "stump_bark", "saw_wood", "saw_bark", "topwood_wood",
  "topwood_bark",
...
.nsvb_biomass_tolerance <- function(precision) {
  # Proposed values. The tolerance table remains maintainer-owned.
  if (identical(precision, "single"))
    0.001 else 2e-12
}
...
.nsvb_evaluate_biomass <- function(data) {
  ...
  .biomass_impl(
    data$DBHOB, data$HTTOT, data$FIASPCD, .nsvb_division_from_id(data$VOLEQ), "nsvb",
    NULL, list(
      region = data$REGN, forest = data$FORST, decay_class = data$DECAYCD, cull = data$CULL,
      primary_top = data$MTOPP, secondary_top = data$MTOPS, stump_ht = data$STUMP,
      max_log_length = ifelse(override,
        data$NEWMAXLEN, NA_real_
      ), min_log_length = ifelse(override, data$NEWMINLEN,
        NA_real_
...
.nsvb_compiled_inverse <- function(data) {
  ...
  for (id in unique(data$VOLEQ)) {
    ...
    evaluated <- merchandiser:::tv_cpp_kernel_eval(
      paste0("nsvb:", id), 2L, data$DBHOB[rows],
      data$HTTOT[rows], data$STEMDIB[rows], data$STEMDIB[rows], rep(NA_real_, length(rows)),
      1L
    )
...
.check_nsvb_fixture <- function(path, precision, full = FALSE, record = FALSE) {
  fixture <- .nsvb_read_fixture(path)

  profile_rows <- grepl("^PROFILE", fixture$CALL_KIND)
  profile <- fixture[profile_rows, , drop = FALSE]
  inside <- .oracle_dib(
    dbh = profile$DBHOB, ht = profile$HTTOT, h = profile$HTUP, model = profile$VOLEQ,
    spcd = .interface_spcd(profile$VOLEQ)
  )
  expect_identical(inside$status, integer(nrow(profile)))
  .nsvb_expect_relative(inside$value, profile$DIB, tv_tolerance[[paste0(
    "diameter_", precision,
    "_rel"
  )]])
  inverse <- .oracle_height_at_dib(
    dbh = profile$DBHOB, ht = profile$HTTOT, dib = profile$STEMDIB,
    model = profile$VOLEQ, spcd = .interface_spcd(profile$VOLEQ)
  )
  stump_diameter <- .oracle_dib(
    dbh = profile$DBHOB, ht = profile$HTTOT, h = 1, model = profile$VOLEQ,
    spcd = .interface_spcd(profile$VOLEQ)
  )
  ...
  expect_identical(inverse$status[!outside_contract], integer(sum(!outside_contract)))
  ...
  source_dob <- .with_treevolume_compat("nvel", .oracle_dob(
    dbh = profile$DBHOB, ht = profile$HTTOT,
    h = profile$HTUP, model = profile$VOLEQ, spcd = .interface_spcd(profile$VOLEQ)
  ))
  ...
  if (record) {
    .gate1_record_values("nsvb", "DIB", precision, inside$value, profile$DIB, rep(
      TRUE, nrow(profile)
    ),
    tv_tolerance[[paste0("diameter_", precision, "_rel")]],
    denominator_floor = 1
    )
    .gate1_record_values("nsvb", "height at DIB", precision, inverse$value, profile$STEMHT,
      !outside_contract, tv_tolerance[[paste0("height_", precision, "_rel")]], list(
        above_contract_stump_diameter = outside_contract
      ),
      denominator_floor = 1
    )
    .gate1_record_values("nsvb", "DOB", precision, source_dob$value, expected_dob, rep(
      TRUE,
      nrow(profile)
    ), 0, denominator_floor = 1, compat = "nvel")
  }
  ...
  green_carbon <- observed$carbon * observed$green_agb_no_foliage /
    observed$dry_aboveground_no_foliage
  actual <- cbind(actual, green_carbon)
  expected <- as.matrix(biomass_fixture[c(paste0("DRYBIO", 1:15), paste0("GRNBIO", 1:15))])
  .nsvb_expect_relative(actual, expected, .nsvb_biomass_tolerance(precision))
  expect_equal(observed$co2e, observed$carbon * 44 / 12, tolerance = 0)
  if (record) {
    tolerance <- .nsvb_biomass_tolerance(precision)
    relative <- abs(actual - expected) / pmax(abs(expected), 1)
    row_within <- apply(is.finite(relative) & relative <= tolerance, 1L, all)
    .gate1_record_summary(
      "nsvb", "biomass, 30 values per row", precision, nrow(relative),
      sum(row_within), max(relative, na.rm = TRUE), tolerance
    )
  }
...
.nsvb_native_biomass <- function(dbh, ht, spcd, division = 0, ...) {
  .biomass_impl(dbh, ht, spcd, division, "nsvb", NULL, list(...), "imperial", TRUE)
}
