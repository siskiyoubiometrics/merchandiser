.reserved_aux_names <- c("status", "measurement_system", "id")

.treevolume_compat <- function() {
  value <- getOption("merchandiser.compat", "port")
  if (!is.character(value) || length(value) != 1L || is.na(value) || !value %in% c(
    "port",
    "nvel"
  )) {
    stop("option merchandiser.compat must be one of: port, nvel.", call. = FALSE)
  }
  value
}
...
.auxiliary_table <- data.frame(
  name = c(
    ...
    "site_index", "basal_area", "bark_ratio", "decay_class", "cull", "upper_bark", "spcd"
  ), type = c(rep(
    "numeric",
    10L
  ), "character", "numeric"), dimension = c(
    "dimensionless", "height", "diameter", "height",
    "diameter", "height", "area", "dimensionless", "dimensionless", "dimensionless",
    "dimensionless",
    "dimensionless"
  ), domain = c(
    "positive", "positive", "positive", "positive", "positive",
    "positive", "positive", "fraction", "integer_1_5", "percentage", "bark_basis",
    "positive_integer"
  ),
...
.common_size <- function(values, aliases = character()) {
  sizes <- vapply(values, length, integer(1))
  for (name in intersect(names(sizes), names(aliases))) {
    names(sizes)[names(sizes) == name] <- aliases[[name]]
  }
  if (!length(sizes)) {
    return(1L)
  }
  if (any(sizes == 0L)) {
    if (all(sizes %in% c(0L, 1L))) {
      return(0L)
    }
    .recycling_error(sizes, which(sizes == 0L)[1L], max(sizes))
  }
  non_scalar <- unique(sizes[sizes != 1L])
  if (length(non_scalar) > 1L) {
    .recycling_error(sizes, which(sizes != 1L & sizes != max(sizes))[1L], max(sizes))
  }
  if (length(non_scalar))
    non_scalar else 1L
}

.recycling_error <- function(sizes, index, common) {
  stop(names(sizes)[index], " has size ", sizes[index], " but the common size is ", common,
    ". Only size-one vectors can be recycled.", call. = FALSE
  )
}
...
.prepare_vectors <- function(values, numeric_names, character_names, aux, aliases = character()) {
  aux <- .capture_aux(aux)
  size <- .common_size(c(values, aux), aliases)
  ...
  for (name in numeric_names) {
    if (!is.numeric(values[[name]])) {
      label <- if (name %in% names(aliases)) aliases[[name]] else name
  ...
  for (name in character_names) {
    ...
    if (!is.character(values[[name]]) && !is_model_factor) {
      label <- if (name %in% names(aliases)) aliases[[name]] else name
      stop(label, " must be character.", call. = FALSE)
    }
  }
...
.dictionary_groups <- function(ids, size = length(ids)) {
  if (length(ids) == 1L) {
    return(list(dictionary = as.character(ids), encoded = 1L, groups = stats::setNames(
      list(seq_len(size)),
      "1"
    )))
  }
  ...
  list(dictionary = dictionary, encoded = encoded, groups = split(seq_along(ids), encoded))
}
...
.auxiliary_domain_invalid <- function(value, domain) {
  switch(domain,
    ...
    integer_1_5 = value !=
      floor(value) | value < 1 | value > 5,
    percentage = value < 0 | value > 100,
    bark_basis = !value %in%
      c("ib", "ob"),
    positive_integer = value != floor(value) | value <= 0 | value > .Machine$integer.max,
...
.validate_auxiliary_values <- function(name, value, rows, status) {
  ...
  if (!type_ok) {
    stop("auxiliary ", name, " has ", length(rows), " values of the wrong type.",
      call. = FALSE
    )
  }
  ...
  invalid_rows <- rows[invalid]
  status[invalid_rows[status[invalid_rows] %in% c(0L, 52L)]] <- 55L
  status
}
...
.resolve_models <- function(ids, aux, status) {
  ...
  if (!is.null(aux$spcd)) {
    if (!is.numeric(aux$spcd)) {
      stop("auxiliary spcd has ", length(status), " values of the wrong type.",
        call. = FALSE
      )
    }
  ...
  for (group_index in seq_along(grouping$dictionary)) {
    ...
    entry <- if (!is.na(id) && nzchar(id))
      .registry_lookup(id) else NULL
    if (is.null(entry)) {
      status[rows[status[rows] == 0L]] <- if (!is.na(id) && !nzchar(id)) 404L else 50L
    ...
    declared_aux <- union(declared_aux, c(model$inputs$required, model$inputs$optional))
    if (!is.null(aux$spcd)) {
      ...
      if (length(model$spcd)) {
        outside <- !invalid & !unknown & !species %in% model$spcd
  ...
  list(
    dictionary = grouping$dictionary, encoded = grouping$encoded, groups = grouping$groups,
    models = models, status = status, details = details
  )
}
...
.auxiliary_to_native <- function(value, dimension, caller_units, native_units) {
  switch(dimension,
    ...
    diameter = .diameter_to_native(
      value,
      caller_units, native_units
    ),
...
.set_aux_caller_units <- function(aux, measurement_system) {
  attr(aux, "caller_units") <- measurement_system
  aux
}

.subset_aux <- function(aux, rows, model) {
  keep <- intersect(names(aux), c(
    model$inputs$required, model$inputs$optional,
    "spcd", "bark_ratio"
  ))
  caller_units <- attr(aux, "caller_units", exact = TRUE)
  if (is.null(caller_units)) {
    caller_units <- model$measurement_system
  }
  ...
  for (name in intersect(keep, .auxiliary_table$name)) {
    ...
    result[[name]] <- .auxiliary_to_native(
      result[[name]], dimension, caller_units,
...
.height_to_native <- function(value, caller_units, native_units) {
  ...
  if (identical(caller_units, "metric"))
    value / 0.3048 else value * 0.3048
}
...
.weight_from_native <- function(value, caller_units, native_units) {
  ...
  if (identical(caller_units, "metric"))
    value * 0.45359237 else value / 0.45359237
}

.step_to_native_height <- function(value, caller_units, native_units) {
  .height_to_native(value, caller_units, native_units)
}
